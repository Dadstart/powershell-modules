class ProgressActivity {
    ...
    hidden [int] _GetPercentComplete() {
        ...
        elseif (($this.TotalItems -gt 0) -and ($this.CurrentItem -ge 0)) {
            return [int]($this.CurrentItem / $this.TotalItems * 100)
        }
        else {
            return -1
        }
    ...
    hidden [void] _UpdateProgressBar() {
        ...
        # Determine what to show for progress
        $progressParams.PercentComplete = $this._GetPercentComplete()
    ...
    [string]ToString() {
        ...
        $sb.Append(') ')
        $sb.Append($this._GetPercentComplete())
        $sb.Append('%')
        return $sb.ToString()
    }
}
